...
| 2026-09-15 | Bug fixes: (1) paid checkout accepted payments for full events when the waitlist was on — PaymentsController now rejects with "This event is sold out." once Confirmed+CheckedIn+Pending ≥ MaxAttendees (paid events have no waitlist; pending payments hold a spot); event-detail Buy Ticket button disabled/labelled Sold Out. (2) Capacity counts everywhere (registeredCount, RegistrationsController, AnalyticsController) now include CheckedIn — checked-in attendees were dropping out of the count and re-opening spots. (3) Notifications page rendered raw email HTML — now shows a stripped plain-text preview (≤220 chars). (4) `$` icons removed: venue detail price and landing feature list used Lucide DollarSign next to Kwacha amounts — replaced with Banknote. (5) Landing brand icon changed from Sparkles to Calendar to match the dashboard navbar. |
